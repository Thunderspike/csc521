...
It does so by taking the input of a text file from the user and running it through
3 functions.
...
def SplitByIdents(source):
    #splits all lexemes by keywords and all other words and numbers
    ...
    while i < len(source):
        ...
        if (line == 'var'):
            ...
            #all r'[\x08]+[.]* take out the \b tokens from the splitting process
            ...
            resword= re.split(r'[\x08]+[.]*', sword)
            allSplits += resword
        elif (matchIdent != None):
            allSplits += ["IDENT: " + matchIdent.group(0)]
        elif (matchNum != None):
            allSplits += ["NUMBER: " + matchNum.group(0)]
        else:
            print('Error with \'{}\'! Your variable name cannot start with a number, or contain any special characters'.format(line))
            sys.exit()
...
if __name__ == '__main__':
    #assigns s to the strings from the file read
    s = sys.stdin.read()
    #calls SpplitSourceByWhitespace, SplitByUnvariedLexemes, and SplitsByIdents
    #in that order, with input s

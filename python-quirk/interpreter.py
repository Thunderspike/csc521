...
This program takes input from the parser.py in a form of a string, turns it
into the tree object through json.load and unpacks the structure of the
recieved parse tree.

It functions by traversing through the tree with the lookup_in_scope_stack
func, and passing NUMBER and IDENT terminals back to python for execution when
found. Terminals are thus used to validate the taken path of the tree returned
from the parser.py as they are specific to the structure. This program will
output a print statement if the tree recieved has a Print0 node.
'''
...
def Program0(pt, scope):
    # Program0 calling the next thing in the pt, and than calling program0
    # again
...
def Program1(pt, scope):
    # calling the next token <Statement> with name, partial parse tree, and
    # scope
    func_by_name(pt[1][0], pt[1], scope)
...
# <FunctionDeclaration> ->
# FUNCTION <Name> PAREN <FunctionParams> LBRACE <FunctionBody> RBRACE
...
def Term1(pt, scope):
    # returning the product value of the left <Term> dividing right
    # <Expression>
    left_value = func_by_name(pt[1][0], pt[1], scope)
    right_value = func_by_name(pt[3][0], pt[3], scope)
    return left_value / right_value
...
def FunctionCall0(pt, scope):
    ...
    list_of_vals = func_by_name(
        function_body_pt[0], function_body_pt, new_scope)
